...
class KGDataset(RecDataset):
    def __init__(self, config):
        super().__init__(config)
        self.dataset_name = config['dataset']
        self.dataset_path = os.path.abspath(config['data_path']+self.dataset_name)

        self.kg_graph = self._load_kg()          # triple (h,r,t)
        self.entity_num = ...                     # n° entità totali
        self.relation_num = ...                   # n° relazioni
        self.item2entity = self._load_link()      # mapping item→entità
    ...
    def _load_link(self):
        # legge baby.link e restituisce dict {item_id: entity_id}
        ...
